# FlexBE States and Behaviors for compare
...
and change the relevant `compare` text to `my_new_project_name` as needed.
...
## Example States in `compare_flexbe_states`
...
## Example Behaviors in `compare_flexbe_behaviors`
...
[`copy_behavior`](compare_flexbe_behaviors/bin/copy_behavior) script to copy a saved behavior 
&mdash; both the Python implementation and manifest `.xml` file &mdash; 
to the project source folder for long term storage.
Use `ros2 run compare_flexbe_behavior copy_behavior` to see the usage guide. 
